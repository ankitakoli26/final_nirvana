...
// ConfidenceLevel enum (backend): INSUFFICIENT_DATA, LOW, MODERATE, HIGH
// [Guessing] — confirm MoodPredictionService.analyzeMood() actually has a
// branch that assigns HIGH (e.g. a log-count threshold). If it doesn't,
// this branch is still unreachable even though the enum value exists.
const confidenceConfig = {
  INSUFFICIENT_DATA: {
    ...
    desc:    'Keep logging daily — predictions unlock soon!'
  },
  LOW: {
    color:   '#3AAFA9',
    ...
    desc:    'Based on 7+ mood logs. Keep logging for better accuracy!'
  },
  MODERATE: {
    color:   '#2E8B57',
    ...
    desc:    'Based on 14+ mood logs. Fairly reliable!'
  },
  HIGH: {
    color:   '#7C3AED',
    ...
    desc:    'Based on 30+ mood logs. Very reliable prediction!'
  }
}
...
// score can legitimately be 0 — never use `score ? x : y` on it directly
const scoreColor = (s) =>
  s === undefined || s === null ? '#666' : s <= 3 ? '#EF4444' : s <= 5 ? '#F59E0B' : s <= 7 ? '#3AAFA9' : '#2E8B57'

const scoreEmoji = (s) =>
  s === undefined || s === null ? '❓' : s <= 2 ? '😞' : s <= 4 ? '😔' : s <= 6 ? '😐' : s <= 8 ? '🙂' : '😄'
...
export default function Insights() {
  ...
  useEffect(() => {
    getMoodPrediction()
      ...
      .catch(err => {
        console.error('❌ Mood prediction fetch failed:', err.response?.data || err.message)
        setError(true)
      })
      .finally(() => setLoading(false))
  }, [])

  // Backend shape: PatientCycleInsightDto { data: PatientCycleDto, narrativeInsight: string }
  // PatientCycleDto { weeklyPattern: Map<DayOfWeek, Double>, lowestDay, highestDay, tomorrowEstimate, confidence }
  ...
  const hasTomorrow    = tomorrow !== undefined && tomorrow !== null
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-sm mt-3" style={{ color:'rgba(255,255,255,0.5)' }}>
              Couldn't load your mood insights right now. Try refreshing.
            </p>
          </div>
        ) : (
          <>
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-between">
                  ...
                  {confidence === 'INSUFFICIENT_DATA' ? (
                    <div>
                      ...
                      <p className="text-sm" style={{ color:'rgba(255,255,255,0.55)', maxWidth:'450px' }}>
                        {narrative || "Log your mood daily to unlock AI-powered mood predictions!"}
                      </p>
                    </div>
                  ) : (
                    <div>
                ...
                {confidence !== 'INSUFFICIENT_DATA' && hasTomorrow && (
                  <motion.div
            ...
            <div className="grid grid-cols-3 gap-5 mb-8">
              ...
              >
                ...
                <div className="text-xs" style={{ color:'rgba(255,255,255,0.4)' }}>
                  {highestDay && weeklyPattern[highestDay] !== undefined
              ...
              >
                ...
                <div className="text-xs" style={{ color:'rgba(255,255,255,0.4)' }}>
                  {lowestDay && weeklyPattern[lowestDay] !== undefined
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              ...
              >
                ...
                {Object.keys(dayColors).map((day, i) => {
                  ...
                  const color = hasData ? scoreColor(score) : 'rgba(255,255,255,0.15)'
